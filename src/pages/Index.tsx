...
const INQUIRY_BUYERS: InquiryBuyer[] = [
  {
    ...
    stage: "深度跟进",
    stageTone: "primary",
    nextAction: { label: "跟进进展，提供最新报价方案" },
    analyses: [
  ...
  },
  {
    ...
    stage: "首次询盘",
    stageTone: "primary",
    nextAction: { label: "获取买家动态更新，捕捉首单决策窗口" },
    analyses: [
                ...
                <div className={cn("grid gap-5", retrainMode ? "lg:max-w-2xl mx-auto" : "lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]")}>
                  ...
                  <div className="relative overflow-hidden rounded-3xl border border-border/60 bg-gradient-to-br from-card/85 via-card/75 to-primary/5 p-6 shadow-xl shadow-primary/5 backdrop-blur-md sm:p-8">
                    ...
                    {retrainMode && trainingStage === "form" && (
                      <div className="relative mt-6 rounded-2xl border border-amber-300/60 bg-amber-50 p-4">
                        <div className="flex items-center gap-1.5 text-[12px] font-semibold text-amber-700">
                          <Sparkles className="h-3.5 w-3.5" />
                          AI 训练摘要
                        </div>
                        <ul className="mt-2.5 space-y-2 text-[13px] leading-relaxed text-amber-900/85">
                          <li className="flex items-start gap-2">
                            <Check className="mt-[3px] h-3.5 w-3.5 shrink-0 text-amber-600" strokeWidth={3} />
                ...
                <div className="mt-1 flex items-center justify-between gap-3">
                  ...
                  <button
                    onClick={() => {
                      ...
                      const isInquirySummary = /汇总.*询盘|询盘.*清单|本周.*询盘|各渠道.*询盘/.test(prefillValue);
